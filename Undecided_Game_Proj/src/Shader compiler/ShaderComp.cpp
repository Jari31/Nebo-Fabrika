...
RID ShaderCompiler::LoadOrCompileShader(const String &path_to_compute_shader, const String &CompileTo, const bool doCompilation,
    ...
    if(doCompilation)
    {
        ...
        if(GDShader_File.is_valid()){
            ...
            String MacroDefinition = String("#define WORKGROUP_SIZE_X ") + String::num_int64(WORKGROUP_SIZE.x) + String("\n") 
                                    + String("#define WORKGROUP_SIZE_Y ") + String::num_int64(WORKGROUP_SIZE.y) + String("\n")
                                    + String("#define WORKGROUP_SIZE_Z ") + String::num_int64(WORKGROUP_SIZE.z) + String("\n"); 
            ...
            case 0:
                ...
                break;
            case 1:
                RenderDeviceShaderFile->set_stage_source(RenderingDevice::SHADER_STAGE_VERTEX, ShaderSource);
                break;
            case 2:
                RenderDeviceShaderFile->set_stage_source(RenderingDevice::SHADER_STAGE_FRAGMENT, ShaderSource);
                break;
            }
            ...
            if(DEBUG){
                ...
                case 0:
                    ...
                    break;
                case 1:
                    err = Shader_SPIRV->get_stage_compile_error(RenderingDevice::SHADER_STAGE_VERTEX);
                    break;
                case 2:
            ...
            if(Shader_SPIRV.is_valid()){
                ...
                {
                case 0:
                    ShaderFile = Shader_SPIRV->get_stage_bytecode(RenderingDevice::SHADER_STAGE_COMPUTE);
                    break;
                case 1:
                    ShaderFile = Shader_SPIRV->get_stage_bytecode(RenderingDevice::SHADER_STAGE_VERTEX);
                    break;
                case 2:
                    ShaderFile = Shader_SPIRV->get_stage_bytecode(RenderingDevice::SHADER_STAGE_FRAGMENT);
                    break;
                }
...
String ShaderCompiler::PreProcessShader(String &Source)
{
    ...
    for(int i = Matches.size() - 1; i >= 0; i--)
    {
        ...
        if(File.is_valid())
        {
            ...
            ERR_PRINT(UtilityFunctions::str("Include directive checks failed at", __LINE__, "! Last processed #include content: ", IncludeDirectiveContent));
        ...
        Matches = RegEx_Local->search_all(TempSource); // it has to search it twice to account for offsets after an insert
    }
