...
pub fn build(Build: *std.Build) void {
    ...
    executable_module.addIncludePath(Build.path("cache/"));
    executable_module.addIncludePath(Build.path("Includes"));
    executable_module.addIncludePath(Build.path("../Includes"));
    executable_module.addIncludePath(Build.path("cache/Libraries/include/reproc/"));
    ...
    GlobFilesWithExtension(Build, Build.path("../cache/Libraries/lib/"), if (target.result.os.tag != .windows) ".a" else ".lib", &library_file_paths);
    GlobFilesWithExtension(Build, Build.path(EXTERNAL_CACHE_FOLDER_LOCATION ++ "Libraries/lib/"), if (target.result.os.tag != .windows) ".a" else ".lib", &library_file_paths);
