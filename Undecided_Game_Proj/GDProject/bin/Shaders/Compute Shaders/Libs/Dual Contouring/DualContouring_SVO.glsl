...
//what buffer has the actual data depends on what pass the SVO algorithm finished at. At pass 1, the predominant buffer is aux; otherwise it is the primary one
...
void main()
{
    ...
    uint TotalNodes = SVO_BufferSize - 1; 
    uint Index = GetMortonCode(ivec3(gl_GlobalInvocationID.xyz));

    {
        uvec3 GID = gl_GlobalInvocationID.xyz;
        if(GID.x > TotalNodes || GID.y > TotalNodes || GID.z > TotalNodes) return;
    }
    ...
    if(PassOffset == 0)
    {
        vec3 NodeMin = vec3(di_x, di_y, di_z) * SVO_VoxelSize;

        vec3 Corners[8];
        Corners[0] = NodeMin;                                                     
        Corners[1] = NodeMin + vec3(SVO_VoxelSize, 0,             0            ); 
        Corners[2] = NodeMin + vec3(0,             SVO_VoxelSize, 0            ); 
        Corners[3] = NodeMin + vec3(SVO_VoxelSize, SVO_VoxelSize, 0            ); 
        Corners[4] = NodeMin + vec3(0,             0,             SVO_VoxelSize);
        Corners[5] = NodeMin + vec3(SVO_VoxelSize, 0,             SVO_VoxelSize);
        Corners[6] = NodeMin + vec3(0,             SVO_VoxelSize, SVO_VoxelSize);
        Corners[7] = NodeMin + vec3(SVO_VoxelSize, SVO_VoxelSize, SVO_VoxelSize);

        float Densities[8];
        float tempVal = 0.0;
        for(int i = 0; i < 4; i++)
        {
            tempVal = UnpackDensity(Node.densities_lower4, i);
            if(tempVal == 0.0)
                tempVal += 1.0;
            Densities[i] = tempVal;
        }
        for(int i = 0; i < 4; i++)
        {
            tempVal = UnpackDensity(Node.densities_higher4, i);
            if(tempVal == 0.0)
                tempVal += 1.0;
            Densities[i + 4] = tempVal; 
        }
        ...
        for(int k = 0; k < 12; k++)
        {
            ...
            float d0 = Densities[i0];
            float d1 = Densities[i1];
        ...
        vec3 Centroid = vec3(0, 0, 0); // or as an alias, position

        // reminder: NodeMin = vec3(di_x, di_y, di_z) * SVO_VoxelSize;
        if(IntersectionCount > 0)
        {
            Centroid = IntersectionSum / float(IntersectionCount);
            const int MAX_ITERATIONS = int(PassNum);
            const float STEP_SCALE = 0.5; 
            const float CONVERGE_EPSILON = 1e-4;

            for(int Iteration = 0; Iteration < MAX_ITERATIONS; Iteration++)
            {
                float fDensity = TrilinearSample(Centroid, NodeMin, SVO_VoxelSize, Densities);
                if(abs(fDensity) < CONVERGE_EPSILON) 
                    break;

                vec3 Gradient = TrilinearGradient(Centroid, NodeMin, SVO_VoxelSize, Densities);
                ...
                Centroid = clamp(Centroid, NodeMin, NodeMin + SVO_VoxelSize);
            }
    ...
    int MC_x = int(di_x); // mc = minimum corner
    int MC_y = int(di_y);
    int MC_z = int(di_z);
    ivec3 MC_xyz = ivec3(MC_x, MC_y, MC_z);

    uint Node_MortonCode = GetMortonCode(MC_xyz);
    uint Node_Owner_MortonCode = Node_MortonCode;
    uint EdgeMask = uint(Node_EdgeMask[Index]);
    if(EdgeMask <= 0) return;

    ivec3 NeighborPosition[4];
    uint NeighborNode_MortonCode[4];
    ...
    for(int i = 0; i < 12; i++)
    {
        ...
        for(int j = 0; j < 4; j++)
        {
            ...
            NeighborNode_MortonCode[j] = GetMortonCode(NeighborPosition[j]);
            if(NeighborNode_MortonCode[j] < Node_MortonCode) 
                Node_MortonCode = NeighborNode_MortonCode[j];
        }

        if(Node_MortonCode != Node_Owner_MortonCode) continue;

        int NodeIndices[4];
        for(int j = 0; j < 4; j++)
        {
            int Node_Index = FindNodeByMortonCode(NeighborNode_MortonCode[j], TotalNodes);
            ...
            NodeIndices[j] = int(NeighborNode_MortonCode[j]);
        }
    ...
    for(int i = 0; i < 3; i++)
    {
        if((EdgeMask & (1u << i)) == 0) continue;

        ivec3 Offsets;
        if(i == 0)
            Offsets = {ivec3(0,0,0), ivec3(0,-1,0), ivec3(0,-1,-1), ivec3(0,0,-1)};
        else if(i == 1)
            Offsets = {ivec3(0,0,0), ivec3(0,0,-1), ivec3(-1,0,-1), ivec3(-1,0,0)};
        else
            Offsets = {ivec3(0,0,0), ivec3(-1,0,0), ivec3(-1,-1,0), ivec3(0,-1,0)};
        ...
        for(int j = 0; j < 4; j++)
        {
            ivec3 SamplePosition = ivec3(MC_x, MC_y, MC_z) + Offsets[j];
            ...
            uint MortonCode = GetMortonCode(SamplePosition);
            Node_Indices[j] = FindNodeByMortonCode(MortonCode, TotalNodes);

            if(Node_Indices[j] < 0 || Node_VertexIndex[Node_Indices[j]] < 0){
                Exists = 0;
                break;
            }
        }
        ...
        int Vertex_0 = Node_VertexIndex[Node_Indices[0]];
        int Vertex_1 = Node_VertexIndex[Node_Indices[1]];
        int Vertex_2 = Node_VertexIndex[Node_Indices[2]];
        int Vertex_3 = Node_VertexIndex[Node_Indices[3]];
